...
// ============================
// OPEN OSEK ZEIR ONLINE — Completion / payment funnel
// Duplicated from OpenOsekPaturOnline with full zeir copy variant.
// Kept fully separated: any copy, slug, tracking, and CRM payload
// references "עוסק זעיר" only — never "עוסק פטור".
// ============================
...
export default function OpenOsekZeirOnline() {
    ...
    if (step === 4 && !thtk && !paymentLoading && !paymentError) {
      ...
      (async () => {
        try {
          ...
          // Stash zeir-specific lead payload so handlePaymentSuccess can
          // submit it after the charge completes. service_type / pageSlug
          // must stay "osek_zeir" throughout so the bot speaks zeir.
          try {
          ...
          const handshakeResult = await invokeFunction('tranzilaHandshake', {
            sum: 299,
            // מבטיח ש-product_type / product_name ב-payments יישמרו כ"עוסק זעיר"
            // ולא כ"עוסק פטור" (ברירת המחדל של ה-function).
            service_type: 'osek_zeir',
          });
  ...
  const handlePaymentSuccess = async (txData) => {
    ...
    try {
      ...
      if (pending && tranzilaPhone) {
        // IMPORTANT: pageSlug is the zeir-specific slug. This is what
        // routes the bot flow into zeir-variant copy via the intent
        // classifier. Do NOT change this to the patur slug.
        await invokeFunction('submitLeadToN8N', {
          ...pending,
          phone: tranzilaPhone.replace(/\D/g, '').replace(/^972/, '0'),
          pageSlug: 'open-osek-zeir-online',
          message: 'שולם 299 ₪ — עוסק זעיר אונליין',
    ...
    // Tracking — zeir-specific content_name for clean campaign attribution
    window.dataLayer?.push({
      event: 'purchase',
      value: 299,
      currency: 'ILS',
      service_type: 'osek_zeir',
      content_name: 'open_osek_zeir_online',
    });
  ...
  return (
    <>
      <Helmet>
        <title>פתיחת עוסק זעיר אונליין | פרפקט וואן</title>
        <meta name="description" content="השלמת תהליך פתיחת עוסק זעיר אונליין — מסלול מהיר לעצמאים מתחילים. 4 שלבים קצרים ותשלום מאובטח." />
            ...
            <AnimatePresence mode="wait" custom={dir}>
              ...
              {step === 0 && (
                ...
                >
                  <h1 className="text-2xl sm:text-3xl font-bold text-gray-900 leading-tight">
                    פתיחת עוסק זעיר אונליין
                    <br />
                    <span className="text-blue-600">נשארו כמה פרטים קטנים</span>
                  </h1>
                  <p className="text-gray-600 text-lg">
                    זה לוקח פחות מ-2 דקות ואנחנו מתחילים לטפל במסלול הזעיר שלך מיד
                  </p>
              ...
              {step === 2 && (
                ...
                >
                  ...
                  <FieldGroup label="צפי הכנסה חודשי" error={errors.income}>
                    ...
                    <p className="text-xs text-gray-400 mt-1">עוסקים זעירים לרוב מתחילים עד 10,000 \u20AA</p>
                  </FieldGroup>
              ...
              {step === 3 && (
                ...
                >
                  ...
                  <FieldGroup label="צילום תעודת זהות" error={errors.file}>
                    ...
                    <p className="text-xs text-gray-400 mt-1">נדרש לצורך פתיחת תיק העוסק הזעיר ברשויות</p>
                  </FieldGroup>
                ...
                >
                  ...
                  <div className="bg-white border border-gray-200 rounded-2xl p-6 shadow-sm space-y-4">
                    <div className="text-center">
                      ...
                      <p className="text-gray-600 font-medium mt-1">תשלום חד-פעמי למסלול עוסק זעיר — בלי מנוי, בלי הפתעות</p>
                    </div>

                    <div className="bg-blue-50 rounded-xl p-4 space-y-2">
                      <p className="text-sm font-bold text-blue-900">מה קורה אחרי התשלום?</p>
                      <ul className="space-y-2 text-sm text-blue-800">
                        <li className="flex items-center gap-2"><CheckCircle2 className="w-4 h-4 text-green-500 shrink-0" /> <strong>פתיחת תיק עוסק זעיר</strong> — מול הרשויות הרלוונטיות</li>
                        <li className="flex items-center gap-2"><CheckCircle2 className="w-4 h-4 text-green-500 shrink-0" /> <strong>תוך 72 שעות</strong> התיק הזעיר שלך פתוח ומוכן</li>
                        <li className="flex items-center gap-2"><CheckCircle2 className="w-4 h-4 text-green-500 shrink-0" /> <strong>איש קשר בוואטסאפ</strong> — זמין לכל שאלה על עוסק זעיר</li>
                      </ul>
                    </div>

                    <div className="bg-amber-50 border border-amber-200 rounded-xl p-3 text-center">
                      <p className="text-sm text-amber-800 font-semibold">⚡ עצמאים מתחילים חדשים מצטרפים אלינו כל יום</p>
                    </div>
                  </div>
                  ...
                  {thtk && tranzilaSupplier && (
                    <>
                      ...
                      >
                        ...
                        <input type="hidden" name="pdesc" value="פתיחת עוסק זעיר אונליין" />
              ...
              {step === 5 && (
                ...
                >
                  ...
                  <h2 className="text-2xl font-bold text-gray-900">הבקשה לעוסק זעיר התקבלה בהצלחה!</h2>
                  <p className="text-gray-600 text-lg">אנחנו מתחילים לעבוד על פתיחת העוסק הזעיר שלך</p>
                  <div className="bg-blue-50 rounded-xl p-4">
                    <p className="text-blue-700 font-medium">נחזור אליך תוך 24 שעות עם עדכון על מסלול הזעיר</p>
                  </div>
                </motion.div>
              )}
            </AnimatePresence>
