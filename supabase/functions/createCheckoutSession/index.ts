...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (product_type === 'cart') {
      if (!items || items.length === 0) return errorResponse('Cart is empty', 400, req);
      ...
      if (product_type === 'plan') {
        ...
        if (!plans?.length) return errorResponse('Plan not found', 404, req);
        product = plans[0];
        ...
        if (!goals?.length) return errorResponse('Goal not found', 404, req);
        product = goals[0];
        amount = 9900;
        ...
        if (!pages?.length) return errorResponse('Landing page not found', 404, req);
        product = pages[0];
        amount = 29900;
    ...
    if (payErr) return errorResponse(payErr.message, 500, req);
    ...
    if (!session.id) return errorResponse('Failed to create Stripe session', 500, req);
